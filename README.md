...
```
After adding library, some of most useful libraries (such as `Androidx AppCompat` , `Coroutine` , `Glide` , `Kodein` , `Kotlinx Serialization` , `Material Components` , `Moshi` , `Navigation Components` , `Retrofit` ) will be added in your app. So you don't need add this libraries manually.
...
class App : MeowApp() {
	...
	// Dependency Injection create a kodein module
...
Main Activity is a sample activity that needs Kodein Dependency Injection and ViewDataBinding and View Model so see this sample  :
```kotlin
